...
const run = (args: string) => {
  ...
  const close = Rx.fromEvent<[number | null, NodeJS.Signals | null]>(
    child,
    "close"
  );
  const log = Rx.merge(
...
describe("exiting conditions", () => {
  ...
  it("is of success when --success=first and first command to exit succeeds", (done) => {
    run('--success=first "echo foo" "sleep 0.5 && exit 1"').close.subscribe(
      (exit) => {
        expect(exit[0]).toBe(0);
        done();
      },
  ...
  it("is of failure when --success=first and first command to exit fails", (done) => {
    run('--success=first "exit 1" "sleep 0.5 && echo foo"').close.subscribe(
      (exit) => {
        expect(exit[0]).toBeGreaterThan(0);
        done();
      },
  ...
  it("is of success when --success=last and last command to exit succeeds", (done) => {
    run('--success=last "exit 1" "sleep 0.5 && echo foo"').close.subscribe(
      (exit) => {
        expect(exit[0]).toBe(0);
        done();
      },
  ...
  it("is of failure when --success=last and last command to exit fails", (done) => {
    run('--success=last "echo foo" "sleep 0.5 && exit 1"').close.subscribe(
      (exit) => {
        expect(exit[0]).toBeGreaterThan(0);
        done();
      },
...
describe("--prefix-length", () => {
  ...
  it("specifies custom prefix length", (done) => {
    const child = run(
      '--prefix command --prefix-length 5 "echo foo" "echo bar"'
    );
    child.log.pipe(buffer(child.close)).subscribe((lines) => {
      expect(lines).toContainEqual(expect.stringContaining("[ec..o] foo"));
      expect(lines).toContainEqual(expect.stringContaining("[ec..r] bar"));
...
describe("--handle-input", () => {
    ...
    child.close.subscribe((exit) => {
      ...
      expect(lines).toContainEqual(
        expect.stringMatching(
          createKillMessage("[0] node fixtures/read-echo.js")
        )
      );
      done();
    }, done);
  ...
  it("forwards input to specified process", (done) => {
    const lines: string[] = [];
    const child = run(
      '-ki "node fixtures/read-echo.js" "node fixtures/read-echo.js"'
    );
    child.log.subscribe((line) => {
      lines.push(line);
      if (/\[1\] READING/.test(line)) {
...
describe("--timings", () => {
  const defaultTimestampFormatRegex =
  ...
  const expectLinesForProcessStartAndStop = (
    lines: string[],
    index: number,
    command: string
  ) => {
  ...
  const expectLinesForTimingsTable = (lines: string[]) => {
    ...
    const tableHeaderRowRegex =
      /(\W+(name|duration|exit code|killed|command)\W+){5}/g;
